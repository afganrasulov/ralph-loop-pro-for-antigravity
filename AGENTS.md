...
Ralph Loop Pro for Antigravity is a VSCode extension that enables autonomous AI agent loops using the "Ralph Loop" methodology. It interfaces with Antigravity (Codeium's AI coding assistant) via its internal gRPC-Web API to run iterative coding sessions with fresh context per iteration.

**Repository:** [afganrasulov/ralph-loop-pro-for-antigravity](https://github.com/afganrasulov/ralph-loop-pro-for-antigravity). The public-facing issues repo is at `Issues_repo/ralph-loop-for-antigravity/` for community bug reports and feature requests.
...
Model IDs for Antigravity are managed via a **dynamic model registry** (`antigravityClient/modelRegistry.js`). The registry fetches models from a remote `models.json` hosted at [afganrasulov/ralph-loop-models](https://github.com/afganrasulov/ralph-loop-models). To add new models, update that repo — no code changes needed. Hardcoded fallbacks remain in `antigravityClient/protobuf.ts` (`MODEL_IDS`).
